...
//controls which screen navigates to which one
//constantly runs and is called when a go back or go forwards button pressed anywhere
...
class _CollectionStateMachineState extends State<CollectionStateMachine> {
  ...
  VoidCallback switchToNewPageState(PageState newPageState) {
    setState(() {
      _currentPageState = newPageState;
    });
    return null;
  }
  ...
  void didChangeDependencies() {
    ...
    if (_currentPageState == PageState.SENSITISATION) {
      if (args?.containsKey('consumptionData') ??
          false) if (args["consumptionData"] != null)
        consumptionData = args["consumptionData"];
      if (args?.containsKey('initialPageState') ??
          false) if (args["initialPageState"] != null)
        _currentPageState = args["initialPageState"];
    }
  ...
  Widget build(BuildContext context) {
    ...
    Map<PageState, dynamic> PageStateWidgetMap = {
      PageState.SENSITISATION: SensitisationVisitDataCard(
        ...
        updatePageState: (interviewData) =>
            {consumptionData.interviewData = interviewData},
      ...
      PageState.INTERVIEW: InfoDataCard(
        navigatePageStateForward: () {
          switchToNewPageState(PageState.FIRST_PASS);
        },
        navigatePageStateBackward: () {
          switchToNewPageState(PageState.SENSITISATION);
        },
        updatePageState: (interviewData) =>
            {consumptionData.interviewData = interviewData},
        initialInterviewData: consumptionData.interviewData,
      ...
      PageState.FIRST_PASS: FoodItemList(
        navigatePageStateForward: () {
          switchToNewPageState(PageState.SECOND_PASS);
        },
        navigatePageStateBackward: () {
          switchToNewPageState(PageState.INTERVIEW);
        },
        navigateDummy: () {
          switchToNewPageState(PageState.DUMMY);
        },
        navigateHome: () {
          Navigator.pop(context);
        },
        updatePageState: (foodItemsList) {
          consumptionData.listOfFoods = foodItemsList;
        },
      ...
      PageState.SECOND_PASS: FoodItemList2(
        navigatePageStateForward: () {
          switchToNewPageState(PageState.PROBE_LIST);
        },
      ...
      PageState.THIRD_PASS: RecipeItemList(
        ...
        navigatePageStateForward: () {
          switchToNewPageState(PageState.FOURTH_PASS);
        },
        navigateHome: () {
          Navigator.pop(context);
        },
      ...
      PageState.FOURTH_PASS: FinalReportCard(
        ...
        updatePageState: (consumptionData) {
          consumptionData = consumptionData;
        },
        consumptionData: consumptionData,
      ),
    ...
    return new Scaffold(
      ...
      drawer: CollectionCommonDrawer(
        goToPageStateSensitisation: () =>
            switchToNewPageState(PageState.SENSITISATION),
        goToPageStateInfo: () => switchToNewPageState(PageState.INTERVIEW),
        goToPageStateFirstPage: () =>
            switchToNewPageState(PageState.FIRST_PASS),
        goToPageStateSecondPage: () =>
            switchToNewPageState(PageState.SECOND_PASS),
        goToPageStateThirdPage: () =>
            switchToNewPageState(PageState.THIRD_PASS),
        goToPageStateFourthPage: () =>
            switchToNewPageState(PageState.FOURTH_PASS),
      ),
    );
  }
